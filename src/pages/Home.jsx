...
function Home() {
  ...
  const [searchQuery, setSearchQuery] = useState('');
   ...
   const getItem =async ()=>{
    const resposnce = await fetch('https://housyy.onrender.com/sp/getsps',{
      method: 'GET',
      headers: {
   ...
   useEffect(()=>{
    ...
    const handleSearch = async () => {
      ...
      try {
        const response = await fetch(`https://housyy.onrender.com/sp/searchProviders?latitude=${selectedLocationObj.latitude}&longitude=${selectedLocationObj.longitude}&query=${searchQuery}`);
